...
import { X, Calendar, Award, Shield, Eye } from 'lucide-react';
              ...
              <div className="relative z-10 flex flex-col gap-3 sm:gap-4">
                ...
                >
                  {certificate.verified && (
                    <span className="px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium border bg-emerald-500/20 text-emerald-400 border-emerald-500/30 flex items-center gap-1">
                      <Shield className="w-3 h-3" />
                      {t('certificates.verified')}
                    </span>
                  )}
                  <span className="text-starWhite/60 text-xs sm:text-sm flex items-center gap-1">
                    <Calendar className="w-3 h-3" />
                    {t('certificates.issued')} {certificate.issuedDate}
                  </span>
                </motion.div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6 lg:gap-8">
                ...
                <div className="space-y-3 sm:space-y-4 md:space-y-6">
                  ...
                  <motion.div
                    ...
                    transition={{ duration: 0.2, delay: 0.15 }}
                  >
                    <h3 className="text-sm sm:text-base md:text-lg font-semibold mb-2 sm:mb-3 md:mb-4 text-white flex items-center gap-2">
                      <Eye className="w-4 h-4 text-nebulaPink" />
                      {t('certificates.credentialDetails')}
                    </h3>
                    <div className="space-y-2 sm:space-y-3 md:space-y-4">
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        <Award className="w-3 h-3 sm:w-4 sm:h-4 text-cosmicBlue flex-shrink-0" />
                        <span className="text-starWhite/70 text-xs sm:text-sm">
                          <span className="font-medium">{t('certificates.credential')}:</span> {certificate.credentialId}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-nebulaPink flex-shrink-0" />
                        <span className="text-starWhite/70 text-xs sm:text-sm">
                          <span className="font-medium">{t('certificates.issued')}:</span> {certificate.issuedDate}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        <Shield className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-500 flex-shrink-0" />
                        <span className="text-starWhite/70 text-xs sm:text-sm">
                          <span className="font-medium">{t('certificates.verificationStatus')}:</span> {certificate.verified ? t('certificates.verified') : t('certificates.notVerified')}
                        </span>
                      </div>
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    >
                      ...
                      <span>{t('certificates.viewFullCertificate')}</span>
                    </motion.button>
                    ...
                    >
                      ...
                      <span>{t('certificates.downloadCertificate')}</span>
                    </motion.button>
                  </motion.div>
                </div>
              </div>
